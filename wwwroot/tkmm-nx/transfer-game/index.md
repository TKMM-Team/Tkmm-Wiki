---
_appTitle: ~
title: Transfer TotK from System Memory to microSD Card
description: Installation guide for TKMM-NX
---

# Transfer TotK from System Memory to microSD Card

<br>

In order to transfer the game from system memory to the SD card, open to the Switch settings and go to the `Data Management` section, then select the option `Move Data Between System/microSD Card`

<p>
    <img width="650" src="../images/TransferToSD1.png" alt="Transfer to SD card (scr 1)">
</p>

On that menu, select the option `Move to microSD Card`

<p>
    <img width="650" src="../images/TransferToSD2.png" alt="Transfer to SD card (scr 2)">
</p>

Finally, select `The Legend of Zelda: Tears of the Kingdom` in the list of games, and press Move Data

<p>
    <img width="650" src="../images/TransferToSD3.png" alt="Transfer to SD card (scr 3)">
</p>

<br>
